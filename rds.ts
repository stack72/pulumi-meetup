...
export interface RdsArgs {
    ...
    createCustomDbOptionGroup?: Input<boolean>;
    ...
    numberOfReadReplicas?: Input<number>;

    createCustomDnsRecord?: Input<boolean>;
}
...
export class RdsInstance extends ComponentResource {
    ...
    public instancePort(): Output<number> {
        return this.db.port;
    }
    ...
    constructor(name: string, args: RdsArgs, opts?: ComponentResourceOptions) {
        ...
        let numReplicas = args.numberOfReadReplicas || 0;
        for (let i = 0; i < numReplicas; i++) {
            new aws.rds.Instance(`read-replica-${i}`, {
                instanceClass: "db.t2.micro",
                skipFinalSnapshot: true,

                replicateSourceDb: this.db.identifier,

                tags: {
                    ...args.baseTags,
                    Name: `${args.description} DB Replica Instance`,
                },
            }, {parent: this});
        }
    }
